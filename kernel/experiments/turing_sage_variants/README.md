...
- the `svdint4.turing_sage` public API;
- the default `svdint4-kernel` Python package; and
...
```bash
git worktree add ../svdint4-sage-experiments 2b63f13
cd ../svdint4-sage-experiments/kernel
SVDINT4_ARCH_LIST=7.5 python -m pip install .
```
...
`SVDINT4_EXPERIMENTAL_SAGE_VARIANTS` macro is deliberately not enabled by the
production `setup.py`.
