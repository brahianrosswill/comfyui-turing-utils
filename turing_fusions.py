...
def turing_linear_group(
    linears: Sequence[torch.nn.Module], x: torch.Tensor
) -> tuple[torch.Tensor, ...] | None:
    """Quantize one shared ConvRot activation and apply several compatible linears."""
    import comfy.model_management
    import comfy.ops
    import comfy.quant_ops

    try:
        from . import turing_ops
    except ImportError:
        import turing_ops
    ...
    if (
        ...
        or comfy.model_management.in_training
        or not turing_ops.is_supported_turing_device(x.device)
    ):
        return None
    declared_kinds = [convrot_weight_kind(getattr(linear, "weight", None)) for linear in linears]
    if declared_kinds[0] is None or any(kind != declared_kinds[0] for kind in declared_kinds):
        return None
    ...
    try:
        for linear in linears:
            ...
            casted.append((linear, weight, bias, offload))
        kinds = [convrot_weight_kind(weight) for _, weight, _, _ in casted]
        if kinds[0] is None or any(kind != kinds[0] for kind in kinds):
            return None

        original_shape = x.shape
        x2d = x.reshape(-1, original_shape[-1]).contiguous()
        kind = kinds[0]
        if kind in ("w8a8", "w4a8"):
            qactivation, activation_scale = turing_ops._quantize_turing_int8_activation(
                x2d, 256
            )
        else:
            qactivation, activation_scale = turing_ops._quantize_turing_int4_activation(
                x2d, 256
            )

        outputs = []
        for _, weight, bias, _ in casted:
            if kind == "w8a8":
                qweight, weight_scale = comfy.quant_ops.TensorWiseINT8Layout.get_plain_tensors(weight)
                ...
                qweight, weight_scale = comfy.quant_ops.TensorCoreConvRotW4A4Layout.get_plain_tensors(weight)
                if kind == "w4a8":
                    from comfyui_turing_utils_kernel import turing_w4a8_linear

                    output = turing_w4a8_linear(
                        qactivation, qweight, activation_scale, weight_scale, bias
                    )
                else:
            ...
            outputs.append(
                output.reshape(*original_shape[:-1], int(weight.shape[0]))
            )
        return tuple(outputs)
    finally:
        for linear, weight, bias, offload in reversed(casted):
