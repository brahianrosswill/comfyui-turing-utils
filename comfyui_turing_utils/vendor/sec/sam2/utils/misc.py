...
# Modified by Turing Utils: added lossless tensor-backed video-frame loading.
...
def load_video_frames(
    ...
    if is_bytes or is_mp4_path:
        ...
        raise NotImplementedError(
            "Only IMAGE tensors, MP4 video, and JPEG folders are supported"
        )
...
def load_video_frames_from_tensor(
    ...
    """Prepare a ComfyUI IMAGE batch without a lossy disk round-trip.

    ``frames`` is expected in ComfyUI's ``[N,H,W,C]`` layout.  The raw batch
    remains owned by the caller; this function creates only the normalized
    square tensor consumed by the SAM2 image encoder.

    This file is derived from Comfyui-SecNodes and modified by Turing Utils.
    """
    if frames.ndim != 4 or frames.shape[-1] < 3 or frames.shape[0] < 1:
        raise ValueError(
            "SeC tensor video must have shape [frames,height,width,channels>=3], "
            f"got {tuple(frames.shape)}"
        )

    video_height, video_width = int(frames.shape[1]), int(frames.shape[2])
    images = frames[..., :3].detach().to(device="cpu", dtype=torch.float32)
    ...
    mean = torch.tensor(img_mean, dtype=torch.float32, device=target_device)[:, None, None]
    std = torch.tensor(img_std, dtype=torch.float32, device=target_device)[:, None, None]
