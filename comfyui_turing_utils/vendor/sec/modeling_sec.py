...
class SeCModel(PreTrainedModel):
    ...
    @property
    def device(self):
        # Turing Utils: DynamicVRAM keeps stored parameters on the offload
        # device, so Hugging Face's first-parameter heuristic does not describe
        # the device on which model operations execute.
        comfy_load_device = getattr(self, '_comfy_load_device', None)
        if comfy_load_device is not None:
    ...
    def propagate_in_video(
        ...
        video_paths = inference_state.get("video_paths")
        video_frames = inference_state.get("video_frames")
        ...
        def get_frame(frame_idx):
            ...
            if video_frames is not None:
                ...
                cached = Image.fromarray(array, mode="RGB")
            else:
                cached = Image.open(video_paths[frame_idx]).convert("RGB")
            frame_cache[frame_idx] = cached
            return cached
        ...
        for frame_idx in tqdm(processing_order, desc="propagate in video"):
            ...
            if frame_idx in consolidated_frame_inds["cond_frame_outputs"]:
                ...
                current_img = get_frame(frame_idx)
                last_img = get_frame(max(0, frame_idx - 1))
                flags = [is_scene_change_hsv(current_img, last_img)]
                if len(mllm_memory) > mllm_memory_size:
                    if mllm_memory_size <= 1:
                        _mllm_memory = [mllm_memory[0]]
                    else:
                ...
                if True in flags:
                    ...
                    for mem_frame_idx, mem_mask in _mllm_memory:
                        video.append(label_img_with_mask(get_frame(mem_frame_idx), mem_mask))
            ...
            if len(frame_cache) > 10:
                ...
                # inserted frame works for both forward and backward tracking;
                # the upstream numeric-minimum rule grew without bound in reverse.
