...
# Modified by Turing Utils: the predictor accepts ComfyUI IMAGE tensors without
# writing temporary JPEG frames.
...
def build_sam2_video_predictor(
    ...
    **kwargs,
    ...
    # Turing Utils vendors the inference package and its configs together.
    ...
    class SAM2VideoPredictor(base_class):
        def init_state(self, video_path, **kwargs):
            ...
            if torch.is_tensor(video_path):
                inference_state["video_frames"] = (
                    video_path[..., :3]
                    .detach()
                    .to(device="cpu", dtype=torch.float32)
                    .contiguous()
                )
                inference_state["video_paths"] = None
                ...
                if self.use_obj_ptrs_in_encoder:
                    ...
                    if len(pos_and_ptrs) > 0:
                        ...
                        if self.add_tpos_enc_to_obj_ptrs:
                            ...
                            # ``get_1d_sine_pe`` intentionally computes in fp32, while
                            # released SeC checkpoints are commonly fp16/bf16.  Cast at
                            # the learned projection boundary instead of installing the
                            # upstream package's process-wide module hooks.  The object
                            # pointers carry the active device, but upstream may keep
                            # their values in fp32 even for an fp16/bf16 checkpoint.
                            # The learned projection's parameter is the authoritative
                            # dtype at this boundary.
                            projection_weight = getattr(self.obj_ptr_tpos_proj, "weight", None)
                            projection_dtype = (
